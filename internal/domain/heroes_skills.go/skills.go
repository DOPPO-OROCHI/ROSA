...
// СЛАВЯНСКИЙ СВЯЩЕННИК
func (ab SlavicPriestAbilitySpec) Apply(st *game.MatchState, a game.Action) error {
	spec := ab.Spec()
...
// ТЕРРОРИСТ
func (ab BlackCellAbilitySpec) Apply(st *game.MatchState, a game.Action) error {
	spec := ab.Spec()
	p := st.Players[a.PlayerIndex]
	if p == nil {
...
// ИМПЕРСКИЙ КОММАНДИР
func (ab ImperialCommanderAbilitySpec) Apply(st *game.MatchState, a game.Action) error {
	spec := ab.Spec()
	p := st.Players[a.PlayerIndex]
	if p == nil {
		return errors.New("nil player state")
	...
	game.ApplyEffect(u, e)
	return nil
}

// КАРН
func (ab KarnAbilitySpec) Apply(st *game.MatchState, a game.Action) error {
	spec := ab.Spec()
	p := st.Players[a.PlayerIndex]
	if p == nil {
		return errors.New("nil player state")
...
// ВЕРХОВНЫЙ ЛИДЕР
func (ab SupremeLiderAbilitySpec) Apply(st *game.MatchState, a game.Action) error {
	spec := ab.Spec()
	...
	if a.AttackHero {
		...
		if def.HeroHP <= 0 {
			st.Finished = true
			if a.PlayerIndex == 0 {
				st.Result = game.MatchWinP1
			} else {
				st.Result = game.MatchWinP2
			}
		}
		return nil
	}
	...
	if u == nil || slot < 0 {
		return game.ErrDefenderNotFound
	}
	u.HP -= dmg
	if u.HP <= 0 {
		def.RemoveAt(slot)
...
// СИСТЕМА
func (ab TheSystemAbilitySpec) Apply(st *game.MatchState, a game.Action) error {
	spec := ab.Spec()
	...
	if a.AttackHero {
		return game.ErrHeroAbilityCannotAttackHero
	}
	if a.CardInstanceID == "" {
		return game.ErrTargetNotFound
	}
	...
	if right < game.TableSize && def.Table[right] != nil {
